...
interface Props {
  ...
  onDoubleClick?: () => void;
  spanRows?: number;
  displayTransactions?: any[];  // subset de transacciones para mostrar
  isContinuation?: boolean;     // true si es tarjeta de continuación
  hasContinuation?: boolean;    // true si hay más tarjetas después
  className?: string;
}
...
export default function OrderCard({
  ...
  const [canceledItems, setCanceledItems] = useState<{ name: string; qty: number; transactionId: number }[]>([]);
  const [newCanceledItems, setNewCanceledItems] = useState<{ name: string; qty: number; transactionId: number }[]>([]);
  ...
  // ... (rest of component: scrolling logic etc.) ...
  ...
  const saveNotifiedTransaction = useCallback((transactionId: number) => {
    try {
  ...
  const isTransactionNotified = useCallback((transactionId: number) => {
    const notified = getNotifiedTransactions();
  ...
  const saveNotifiedOrder = useCallback((orderId: number) => {
    try {
  ...
  const isOrderNotified = useCallback((orderId: number) => {
    const notified = getNotifiedOrders();
  ...
  // Transacciones a mostrar (subset si es virtual card, o todas si no) filtrando los cancelados de voids
  let transactionsToRender = displayTransactions || order.ordertransactions;
  transactionsToRender = transactionsToRender.filter((t: any) => !t.Combined);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={`flex-1 overflow-hidden bg-gray-50 min-h-0 ${!hasContinuation ? 'pb-4' : ''}`}
      >
